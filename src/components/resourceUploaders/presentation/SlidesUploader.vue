...
if (files.value.length > 0) {
  const file = files.value[0]; // Get the first file in the array
  formData.append("file", file); // Append it to formData as "file"
}

  const url = `${apiUrl}/resources/slides?resourceId=${encodeURIComponent(
    resourceId
