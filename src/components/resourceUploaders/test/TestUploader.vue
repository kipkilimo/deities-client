<template>
  <v-container>
    <v-file-input
      v-model="files"
      accept=".pdf"
      label="Upload PDF Exam Set File"
    ...
    <!-- Upload dialog for questions and answers -->
       <!-- Upload dialog for questions and answers -->
    <v-dialog width="630" v-model="uploadingTestAnswerKeys">
      <v-card title="Upload Exam Marking Guide">
        <v-divider />
        <TestAnswerKeysUploader />
      </v-card>
    </v-dialog> 
  </v-container>
</template>
...
const uploadFiles = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      localStorage.setItem('examQuestonsSet',response.data.url)
      uploadingTestAnswerKeys.value = true;
      return;
