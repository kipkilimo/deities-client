...
const uploadFiles = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      const assignmentAnswersKey = response.data.url;
      const assignmentTaskSet = localStorage.getItem("assignmentTaskSet");
      const assignmentMetaInfo = localStorage.getItem("assignmentMetaInfo");
      const paramsObjRaw = [
        {
          resourceId: resourceStore.resource.id,
          resourceContent: JSON.stringify({
            assignmentAnswersKey: assignmentAnswersKey, // Use the reactive reference here
            assignmentTaskSet: assignmentTaskSet,
      ...
      localStorage.removeItem("assignmentTaskSet");
      localStorage.removeItem("assignmentMetaInfo");
