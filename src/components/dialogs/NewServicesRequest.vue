  ...
  <v-container fluid class="py-5">
    <v-row class="justify-center">
      ...
      <v-col cols="4">
        ...
        <v-card-text>
          <v-select
            :items="amaneDepartments"
            v-model="sentToDepartment"
            density="comfortable"
            class="mt-2"
            label="Service Point"
          ></v-select>
      ...
      <v-col cols="8">
        ...
        <v-card-text>
          <v-select
            :items="departmentServicesArray.slice(1)"
            v-model="requestedServices"
            density="comfortable"
      ...
      <v-card-actions>
        ...
        <v-btn
          color="red"
          @click="invoiceStore.createNewServiceList = false"
          density="compact"
          icon="mdi-close"
        ></v-btn>
      </v-card-actions>
...
// Search input and selected services
const search = ref(""); 

// Computed property to filter services based on search input
const filteredServices = computed(() => {
  return departmentServicesArray.value.filter((service) =>
    service.toLowerCase().includes(search.value.toLowerCase())
  );
});
...
// Clear the search input when a service is selected or cleared
...
const billItems = ref([]);
const rawDepartmentServices = ref([]);
async function buildServicesList() {
  ...
  rawDepartmentServices.value = activeServices;
  const finalVals = activeServices.map((service) => service.__EMPTY);

  departmentServicesArray.value = finalVals;
}
async function buildBilledServicesList() {
  console.log(rawDepartmentServices.value, requestedServices.value);
  function getMatchingItems(data, names) {
    return data.filter((item) => names.includes(item.__EMPTY));
  }

  const names = ["BS FOR MPS", "MRDT"];

  const result = getMatchingItems(rawDepartmentServices.value, requestedServices.value);
  console.log(result);
  billItems.value = result;
}
...
watch(requestedServices, (newVal, oldVal) => {
  if (newVal) {
    // requestedServices.value = [];
    buildBilledServicesList();
  }
});
