    ...
    <v-row>
      <v-col cols="12">
        <v-row align="center" justify="space-between">
          <v-col cols="12" style="width: 100%; padding: 8px">
            <v-row align="center" style="flex-wrap: wrap; margin: 0">
              <v-col
                :cols="isMobile ? 6 : 4"
                class="mt-4"
                style="padding: 0 8px; min-width: 250px"
              >
                <v-text-field
                  v-model="searchQuery"
                  label="Search"
              ...
              <v-col
                ...
                :key="key"
                :cols="isMobile ? 6 : 2"
                style="padding: 0 8px; min-width: 100px"
              >
                <v-select
          ...
          >
            ...
            >
              <v-card
                ...
                target="_blank" 
                @click="selectResource(resource)"
              ><div class="">{{ resource.title }}</div></v-card>
              <!-- <v-row no-gutters>
                  <v-col cols="3">
                    <v-img
                      height="4rem"
...
onBeforeMount(async () => {
  ...
  console.log({ resources: resourceStore.resources });
  console.log({ intialItemId });
  const intialItem = resourceStore.resources.filter(
    (resource) => resource.id === intialItemId
  );
  selectedResource.value = intialItem[0];
  resourceStore.resource = selectedResource.value;
  showMedia.value = true;
});
</script>
