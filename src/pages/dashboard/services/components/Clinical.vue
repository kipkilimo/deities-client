<template>
    <v-container>
            ...
            <v-row v-if="canShowVisits && visitStore.visits.length > 0">
                    ...
                    <v-card :color="visit.patient.personalInfo.gender === 'Male' ? 'info' : 'pink'" variant="tonal"
                        ...
                        <v-card-actions>
                            ...
                            <div class="pa-2 d-flex align-center">
                                ...
                                <v-btn class="me-2 text-none" color="#009ee3"
                                    prepend-icon="mdi-invoice-text-check-outline" variant="outlined"
                                    @click=" visitStore.setActiveVisit(visit), selectVisit(visit, '')" v-if="
                ...
                <v-col cols="4">
                    <v-card outlined style="height: 85vh; overflow-y: auto">
                        ...
                        <v-card-subtitle>
                            <v-row>
                                <v-col cols="8"> <v-card-title class="text-h6 font-weight-bold">
                                ...
                                <v-col cols="4">
                                    <v-btn @click="requestExternalServices(selectedVisit)" class="mt-4 text-none"
                                        color="#009ee3" prepend-icon="mdi-invoice-text-check-outline" variant="text">
                                        View Records
                                    </v-btn>
                                </v-col>
                            ...
                            <v-list dense>
                                ...
                                ">
                                    ...
                                    <v-list-item-content>
                                        ...
                                        <v-col cols="12" v-for="(items, department) in groupedItemsByDepartment"
                                            :key="department">
                                            <v-list dense>
                                                <v-list-item v-for="(item, index) in items" :key="index">
                                                    <v-list-item-content>
                                                        <div class="text-caption text--secondary">
                                                            {{ index + 1 }}. {{ item.description }} ({{ department }})
                                                        </div>
                                                    </v-list-item-content>
                                                </v-list-item>
                                            </v-list>
                        ...
                        <v-card-actions style="position:absolute;">
                            ...
                            <v-tooltip location="bottom">
                                <template #activator="{ props }">
                                    <v-btn v-bind="props" @click="writeNotesOrReport(selectedVisit)"
                                        icon="mdi-card-account-details-outline" variant="text"></v-btn>
                                </template>
                                <span>Write notes or reports</span>
                            </v-tooltip>
                            ...
                            <v-btn @click="requestExternalServices(selectedVisit)" class="me-2 text-none"
                                color="#009ee3" prepend-icon="mdi-invoice-text-check-outline" variant="flat">
                                Request Services
                            </v-btn>

                            <v-btn @click="doSomething(selectedVisit)" color="red" class="text-none"
                                prepend-icon="mdi-account-cancel" variant="text" v-if="
        ...
        <v-dialog v-model="visitStore.createNewDocumentation" width="65%" persistent>
            <v-card>
...
function writeNotesOrReport(visit) {
    // add new invoice actually
    selectedVisit.value = visit;
    visitStore.setActiveVisit(visit);
    visitStore.createNewDocumentation = true;
}
