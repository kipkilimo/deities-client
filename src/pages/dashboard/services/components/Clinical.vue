<template>
  <v-container>
        ...
        <v-col cols="4">
          ...
          >
              ...
              <v-row>
                ...
                <v-col cols="4">
                  <v-btn
                    ...
                    class="mt-4"
                    color="#ff0079"
                    style="color: #ff0079 !important"
                    @click="showPatientRecords(selectedVisit)"
                  ...
                  <v-list-item-content>
                    ...
                    {{ selectedVisit.patient.nextOfKin.fullName }} ({{
                      selectedVisit.patient.nextOfKin.relationSpecify
                            ...
                            <v-list-item-content> 
                   Tel: {{ selectedVisit.patient.nextOfKin.phoneNumber }} | {{
            ...
            <v-card-actions style="position: absolute">
              ...
              <v-btn
                color="#009ee3"
                @click="writeNotes(selectedVisit)"
              ...
              <v-tooltip location="bottom">
                <template #activator="{ props }">
                  <v-btn
                    v-bind="props"
                    color="red"
                    @click="alertCloseVisit(selectedVisit)"
        ...
        >
          ...
          >
            Invoice {{ visitStore.currentVisit.visitNumber }} ({{
              activeInvoice.paid === true ? "PAID" : "UNPAID"
            }})
          </h4>
          <h5 style="margin: 5px 0; text-align: center">
    ...
    <v-dialog v-model="visitStore.createPatientNotes" fullscreen persistent>
      <v-card>
        <WritePatientNotes />
      </v-card>
    </v-dialog>

    <v-dialog v-model="visitStore.viewPatientRecords" fullscreen persistent>
      <v-card>
        <ViewPatientRecords />
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="visitStore.closeCurrentVisitAlert" width="35%" persistent>
      <ConfirmCloseVisit />
    </v-dialog>
  </v-container>
</template>
...
import ViewPatientRecords from "@/components/dialogs/ViewPatientRecords.vue";
...
function writeNotes(visit) {
  // add new invoice actually
  selectedVisit.value = visit;
  visitStore.setActiveVisit(visit);
  visitStore.createPatientNotes = true;
}
...
function alertCloseVisit(visit) {
  ...
  selectedVisit.value = visit;
  visitStore.setActiveVisit(visit);
  visitStore.closeCurrentVisitAlert = true;
}
onBeforeMount(async () => {
...
function showPatientRecords(selectedVisit) {

  visitStore.setActiveVisit(selectedVisit);
  visitStore.viewPatientRecords = true; 
}
